...
NEW_TAGS = []
ASR_ID2TOK = ["<pad>"]+list(string.ascii_lowercase) + [str(x) for x in range(10)] + ["'","-","<space>"] + ["<sos>"] + ["<eos>"]
ASR_ID2TOK.extend(NEW_TAGS)
...
def pad_fac(input, factor=4, device=None):
    add_size = input.size(1) % factor
    if add_size != 0:
        rem_size = factor - add_size
        padd = torch.zeros(input.size(0), rem_size, input.size(2), device=device)
        return torch.cat([input, padd], dim=1)
    else:
        return input
...
def SpecDel(X, lens, train=True):
    ...
    for x in X_:
        ...
        xn.append(x.squeeze(0))
    pack = pack_sequence(xn, enforce_sorted=False)
    Xn, _ = pad_packed_sequence(pack, batch_first=True)
    return Xn

def roll_in(x, lens, fac=4, device=None):
    if fac == 1:
        return x, lens
    lens = [math.ceil(1.*k/fac) for k in lens]
    x = pad_fac(x, fac, device=device)
    if fac == 4:
        full = torch.cat([x, x.roll(-1, dims=1), x.roll(-2, dims=1), x.roll(-3, dims=1)], dim=2)
    else:
        full = torch.cat([x, x.roll(-1, dims=1)], dim=2)
    extract = list(range(0, x.size(1), fac))
...
def convert_tok2id(text):
    ...
    for tok in toks:
        ...
        if tok in NEW_TAGS:
            lst.append(ASR_TOK2ID[tok])
        else:
            for ch in tok:
                lst.append(ASR_TOK2ID[ch])
                end+=1
        ids.append((start,start+end))
        lst.append(ASR_TOK2ID["<space>"])
        start = start+end+1
    return lst[:-1], ids# + [ASR_TOK2ID["<eos>"]]
